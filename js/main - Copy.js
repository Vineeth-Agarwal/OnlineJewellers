var App = {
    launch: function () {
        ...
        for (var i = 0; i < 4; i++) {
            ...
            var prod_quant = document.getElementById("qty" + (i + 1)).value;
            if (prod_quant >= 0) {
                switch (prod_type) {
                    case "earrings":
                        {
                            earrings_cost += prod_quant * a[i];
                            total_cost += prod_quant * a[i];
                            break;
                        }
                    case "bangles":
                        {
                            bangles_cost = prod_quant * a[i] + bangles_cost;
                            total_cost += prod_quant * a[i];
                            break;
                        }
                    case "diamond rings":
                        {
                            rings_cost = prod_quant * a[i] + rings_cost;
                            total_cost += prod_quant * a[i];
                            break;
                        }
                    case "necklaces":
                        {
                            necklaces_cost = prod_quant * a[i] + necklaces_cost;
                            total_cost += prod_quant * a[i];
                            break;
                        }
                    default:
