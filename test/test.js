...
QUnit.test('Testing calculateArea function with several sets of inputs', function (assert) {
    assert.equal(App.computeCost(5, 5), 25, 'Tested with two relatively small positive numbers');
    assert.equal(App.computeCost(-5, -5), 25, 'Tested with two negative numbers. Any arguments less than 1 will be set to 1.');
    assert.equal(App.computeCost(0, 0), 0, 'Tested with two large positive numbers. Any arguments greater than 100 will be set to 100.');
    assert.throws(function () { App.computeCost(null); },'Passing in null correctly raises an Error');
    assert.throws(function () { App.computeCost("Christine","Christine"); }, 'Passing in a string correctly raises an Error');
});
